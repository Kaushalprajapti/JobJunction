...
export const jobseekerDeleteApplication = catchAsyncErrors(async (req, res, next) => {
    ...
    const loguserid = req.user._id.toString();
    const userid = application.applicantID.user.toString();

    //console.log(userid,loguserid);
    if(loguserid != userid)
        {
            return next(new ErrorHandler("You have not access to do delete an application",400));
        }
